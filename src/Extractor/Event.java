...
/** Abstract class designed for making connection with event extraction methods. Extended is method
 * <i>analyzeMessage()</i>, which call extraction methods and set all necessary variables.
 * 
...
abstract public class Event
{
	...
	/** Constructor with email message represented as String.</br> This message must be well
	 * formated and contains all necessary parts (sender, receiver, date, subject and content).
	 * <p>
	 * <i><b>Example of well formated message:</b> </br> From: Filip Ogurcak
	 * &lt;filip.ogurcak@gmail.com&gt; </br>To: filip.ogurcak@gmail.com </br>Date: 1334571973
	 * </br>Subject: aaa </br>Content: The following is the information need for the arch set up.
	 * If you would like to volunteer to help fill out the attachment form* and drop by 1906</i>
	 * 
	 * @param message
	 *              Represent the well formed headers and content of email message, from which
	 *              could be extracted event data
	...
	/** Constructor with specific format of email message.</br> This message contains all necessary
	 * parts (sender, receiver, date, subject and content), so it's easy to use it.
	 * <p>
	 * 
	 * @param message
	...
	/** Set message with specific format of email message.</br> This message contains all necessary
	 * parts (sender, receiver, date, subject and content), so it's easy to use it.
	 * <p>
	 * 
	 * @param message
	...
	/** Method convert message saved in String to specific format of email message. This message
	 * must be well formated and contains all necessary parts (sender, receiver, date, subject and
	 * content).
	 * <p>
	 * <i><b>Example of well formated message:</b> </br> From: Filip Ogurcak
	 * &lt;filip.ogurcak@gmail.com&gt; </br>To: filip.ogurcak@gmail.com </br>Date: 1334571973
	...
	public void parseMessage(String message) throws MessagingException {
		...
		if (matcher.find())
			date = new Date(Long.valueOf(matcher.group().replace("Date: ", "") + "000"));

		matcher = subjectPattern.matcher(message);
		if (matcher.find())
			subject = matcher.group().replace("Subject: ", "");

		matcher = contentPattern.matcher(message);
		if (matcher.find())
			content = matcher.group().replace("Content: ", "");
	...
	/** Abstract method designed for extend. After it, this method provides all extraction
	 * functionality and sets all variables. */
